...
from keras.layers import Dense, Activation ,Conv2D,MaxPooling2D ,Flatten , Conv2DTranspose ,Input,Concatenate,Reshape,ZeroPadding2D
...
def segnet(img_size,classes,optimizer,loss_function,metric):
    ...
    conv2 = Conv2D(kernel_size=(3,3),filters=512,padding='same',kernel_initializer='he_normal')(conv1)
    conv2 = BatchNormalization()(conv2)
    conv2 = Activation('relu')(conv2)
                   
    conv3 = Conv2D(kernel_size=(3,3),filters=512,padding='same',kernel_initializer='he_normal')(conv2)
    ...
    #### Use padding from one side only in order to reverse the mixup caused by maxpooling ####
    pad2 = ZeroPadding2D(padding=((1, 0), (0, 0)))(up2)

                   
    # 256 Volumes
    conv4 = Conv2D(kernel_size=(3,3),filters=512,padding='same'
                   ,kernel_initializer='he_normal')(pad2)
    conv4 = BatchNormalization()(conv4)
    conv4 = Activation('relu')(conv4)

    conv5 = Conv2D(kernel_size=(3,3),filters=512,padding='same',kernel_initializer='he_normal')(conv4)
    conv5 = BatchNormalization()(conv5)
    conv5 = Activation('relu')(conv5)
                   
    conv6 = Conv2D(kernel_size=(3,3),filters=512,padding='same',kernel_initializer='he_normal')(conv5)
